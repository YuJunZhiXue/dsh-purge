...
export function shouldInjectPrompt(_context, _injectOnce = true, _injectText = "") {
  // 0.1.5 in-history：渲染少了注入就会再追加一条 system/message。
  // 用「最后一条是否含注入」决定本步掏空注入，会 A/B 死循环。每步都必须留下完整注入。
  return true;
}
