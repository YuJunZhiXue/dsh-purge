...
function collectInjectText(purgeSections, fallbackText) {
  ...
  const fromMain = String(main?.text || "").trim();
...
export function promptAlreadyInjected(existing, injectText) {
  ...
  return String(existing || "").includes(inject.slice(0, Math.min(80, inject.length)));
}

export function shouldInjectPrompt(context, injectOnce = true, injectText = "") {
  if (injectOnce === false) return true;
  if (isFirstConversationTurn(context)) return true;
  return !promptAlreadyInjected(existingSystemPrompt(context), injectText);
}
...
function normalizeRewriteOptions(fallbackInject, options) {
  if (fallbackInject && typeof fallbackInject === "object" && !Array.isArray(fallbackInject)) {
    return {
      ...
      pinPersonaText: typeof fallbackInject.pinPersonaText === "string" ? fallbackInject.pinPersonaText : "",
    };
  }
  return {
    ...
    pinPersonaText: typeof options?.pinPersonaText === "string" ? options.pinPersonaText : "",
  };
}
...
export function rewritePromptAssembly(assembled, fallbackInject = "", options = {}) {
  ...
  if (opts.pinPersonaText) {
    assembled.sections = [{ name: "deployment:persona-prefix", text: opts.pinPersonaText }];
    return assembled;
  }
  const rest = [];
  const purge = [];
  for (const section of assembled.sections) {
    ...
    if (String(next.name).startsWith("dsh-purge")) {
      if (!opts.fallbackInject) continue;
      purge.push(next);
    } else {
      rest.push(next);
    }
  ...
  const injectText = opts.fallbackInject ? collectInjectText(purge, opts.fallbackInject) : "";
  ...
  const extras = opts.dropPurgeAfterFold
    ...
    : purge;
  assembled.sections = [...folded, ...extras];
  return assembled;
}
