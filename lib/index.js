...
function installIdentityOverride(ctx) {
  ...
  // prepend:false 先于 phase-1 滤镜；prepend:true 在滤镜之后再折一次。
  try {
...
function installWebServer(ctx, cfg) {
  ...
  ctx.inject(["webServer"], (host) => {
      // dsh-purge admin routes perform sensitive actions (restart, config
      // override, patch apply, status). Require the caller to be on the
      // loopback interface so they cannot be invoked over the network
      // without any authentication.
      const registerAdminRoute = host.webServer.register.bind(host.webServer);
      host.webServer.register = (spec, label) => {
        ...
        return registerAdminRoute({
          ...
          handler: async (request, response) => {
            ...
            return innerHandler(request, response);
          },
        }, label);
      };
    host.effect(() => {
...
function isLoopbackRequest(request) {
  const addr = String(request?.socket?.remoteAddress || "");
