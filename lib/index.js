    ...
    host.effect(() => {
      ...
      host.webServer.register({
        ...
        handler: async (request, response) => {
          if (request.method === "GET") {
            try {
              const ui = await core.ensureOverrideContent(core.findDshHome(), false);
              sendJson(response, 200, { ok: true, ...ui });
            } catch (e) {
              sendJson(response, 500, { ok: false, error: String(e) });
            }
            return;
          ...
          if (request.method === "POST") {
            try {
              ...
              const saved = await core.saveOverrideContent(core.findDshHome(), content);
