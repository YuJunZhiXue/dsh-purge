...
function renderSkillLines(items, empty) {
  const out = [];
  if (!items.length) {
    out.push(`  ${empty}`);
    return out;
  ...
  for (const item of items) {
    ...
    if (how) out.push(`      ${how}`);
    if (item.whenToUse) out.push(`      whenToUse ${item.whenToUse}`);
    if (item.files && item.files.length) out.push(`      ${item.files.join(" · ")}`);
    if (item.shadowed) out.push("      工作区有同名，对话里会被盖住");
...
function renderSkillsStatus(st) {
  const user = (st.skills || []).filter((item) => item.scope !== "project");
  const project = (st.skills || []).filter((item) => item.scope === "project");
  const out = [];
  out.push("环境 Skill / $DSH_HOME/skills（这个宿主所有对话）");
  out.push(`  ${st.skills_dir || "$DSH_HOME/skills"}`);
  out.push(...renderSkillLines(user, "(空 — /skills import <压缩包或文件夹>)"));
  out.push("");
  out.push("工作区 Skill / $WORKSPACE/.dsh/skills（当前项目对话，同名盖住环境）");
  out.push(`  ${st.workspace_dir || "$WORKSPACE/.dsh/skills"}${st.workspace_name ? ` · ${st.workspace_name}` : " · 未绑定"}`);
  out.push(...renderSkillLines(project, st.workspace_ready ? "(空 — /skills import <路径> --project)" : "(先 /skills workspace <项目根目录>)"));
  return out.join("\n");
}

function peelSkillFlags(args) {
  const flags = new Set();
  const rest = [];
  for (const arg of args) {
    if (arg === "--project" || arg === "-p" || arg === "project") flags.add("project");
    else if (arg === "--user" || arg === "-u") flags.add("user");
    else rest.push(arg);
  }
  return { flags, rest };
}

async function cliSkillOptions(scope) {
  if (scope !== "project") return { scope: "user" };
  const selected = await skills.pickWorkspace(core.findDshHome());
  if (!selected) throw new Error("未选择工作区。先执行 /skills workspace <项目根目录>");
  return { scope: "project", projectRoot: selected.root };
}
...
async function handleSkillsCommand(rawInput) {
  ...
  switch (sub) {
    ...
    case "new":
    case "c": {
      const peeled = peelSkillFlags(args.slice(1));
      const id = peeled.rest[0];
      if (!id) return { kind: "error", text: "用法: /skills create <id> [说明] [--project]" };
      if (!skills.validSkillName(id)) {
        return { kind: "error", text: "id 必须是 kebab-case，例如 code-review" };
      }
      const description = peeled.rest.slice(1).join(" ").trim() || id;
      try {
        const options = await cliSkillOptions(peeled.flags.has("project") ? "project" : "user");
        const saved = skills.toPublicSkill(dshHome, await skills.saveSkill(dshHome, id, "", description, null, options), options);
        return {
          kind: "success",
    ...
    case "project": {
      ...
      if (!src) {
        const st = await skills.skillsStatus(dshHome);
        return {
          kind: "success",
          text: st.workspace_ready
            ? `当前工作区 ${st.workspace_name}\n  ${st.workspace_dir}`
            : "未绑定工作区。用法: /skills workspace <项目根目录>",
        };
      }
      try {
        const pinned = await skills.setWorkspace(dshHome, { path: src });
        return { kind: "success", text: `✓ 工作区 ${pinned.workspace_name}\n  $WORKSPACE/.dsh/skills` };
      } catch (e) {
        return { kind: "error", text: `绑定失败: ${e.message}` };
      }
    }
    case "import":
    case "i": {
      const peeled = peelSkillFlags(args.slice(1));
      const src = peeled.rest.join(" ").trim();
      if (!src) return { kind: "error", text: "用法: /skills import <压缩包或文件夹> [--project]" };
      try {
        const options = await cliSkillOptions(peeled.flags.has("project") ? "project" : "user");
        const packed = await skills.importFromPath(dshHome, src, options);
        const names = packed.imported.map((item) => item.id).join(", ");
        return { kind: "success", text: `✓ 已导入到${options.scope === "project" ? "工作区 $WORKSPACE/.dsh/skills" : "环境 $DSH_HOME/skills"}：${names}` };
      } catch (e) {
    ...
    case "d": {
      const peeled = peelSkillFlags(args.slice(1));
      const id = peeled.rest[0];
      if (!id) return { kind: "error", text: "用法: /skills delete <id> [--project]" };
      try {
        const options = await cliSkillOptions(peeled.flags.has("project") ? "project" : "user");
        const removed = skills.toPublicSkill(dshHome, await skills.deleteSkill(dshHome, id, options), options);
        return {
          kind: "success",
    ...
    default:
      return {
        ...
        text: "官方 Skill 安装：\n" +
          "  /skills list                      环境 + 工作区\n" +
          "  /skills workspace [项目根目录]     绑定工作区（写入 .dsh/skills）\n" +
          "  /skills create <id> [说明] [--project]\n" +
          "  /skills import <路径> [--project]  压缩包或文件夹\n" +
          "  /skills delete <id> [--project]\n" +
          "  环境：$DSH_HOME/skills（全宿主）\n" +
          "  工作区：$WORKSPACE/.dsh/skills（当前项目，同名优先）\n" +
          "  调用：/<id> 或模型按 description 调 skill 工具",
      };
  }
}
...
export function apply(ctx, config) {
  ...
  if (commands) {
    ...
    commands.register({
      name: "skills",
      description: "官方 Skill：环境 $DSH_HOME/skills，工作区 $WORKSPACE/.dsh/skills。list | workspace [路径] | import <路径> [--project] | create <id> [说明] [--project] | delete <id> [--project] | help",
      input: { hint: "list | workspace [路径] | import <路径> [--project] | create <id> [--project] | delete <id> [--project]" },
...
async function skillOpOptions(dshHome, body) {
  const scope = body?.scope === "project" ? "project" : "user";
  if (scope !== "project") return { scope: "user" };
  const selected = await skills.pickWorkspace(dshHome, body?.workspace || body?.workspaceId);
  if (!selected) throw Object.assign(new Error("未选择工作区"), { status: 400 });
  return { scope: "project", projectRoot: selected.root };
}
...
async function handleSkillsOp(op, body, queryId) {
  ...
  const id = typeof body?.id === "string" ? body.id : String(queryId || "");
  if (op === "status") {
    return { ok: true, ...(await skills.skillsStatus(dshHome, { workspaceId: body?.workspace || body?.workspaceId })) };
  }
  if (op === "workspace") {
    const pinned = await skills.setWorkspace(dshHome, { id: body?.id || body?.workspace, path: body?.path });
    return { ok: true, ...pinned, ...(await skills.skillsStatus(dshHome, { workspaceId: pinned.workspace_id })) };
  }
  const options = await skillOpOptions(dshHome, body);
  if (op === "read") {
    if (!skills.validSkillName(id)) return { status: 400, ok: false, error: "invalid skill name" };
    const item = await skills.readSkill(dshHome, id, options);
    if (!item) return { status: 404, ok: false, error: `skill not found: ${id}` };
    return { ok: true, ...skills.toPublicSkill(dshHome, item, options) };
  }
  if (op === "save" || op === "create") {
    if (!skills.validSkillName(id)) return { status: 400, ok: false, error: "id 必须是 kebab-case，例如 code-review" };
    if (op === "create" && await skills.readSkill(dshHome, id, options)) {
      return { status: 409, ok: false, error: `${id} 已存在` };
    }
    const content = typeof body?.content === "string" ? body.content : "";
    const description = typeof body?.description === "string" ? body.description : "";
    const extra = {};
    if (body && Object.prototype.hasOwnProperty.call(body, "whenToUse")) extra.whenToUse = body.whenToUse;
    if (body && Object.prototype.hasOwnProperty.call(body, "modelInvocable")) extra.modelInvocable = body.modelInvocable !== false;
    if (body && Object.prototype.hasOwnProperty.call(body, "userInvocable")) extra.userInvocable = body.userInvocable !== false;
    const saved = await skills.saveSkill(dshHome, id, content, description, Object.keys(extra).length ? extra : null, options);
    return { ok: true, ...skills.toPublicSkill(dshHome, saved, options) };
  }
  if (op === "delete") {
    if (!skills.validSkillName(id)) return { status: 400, ok: false, error: "invalid skill name" };
    const removed = await skills.deleteSkill(dshHome, id, options);
    return { ok: true, ...skills.toPublicSkill(dshHome, removed, options) };
  }
  if (op === "import") {
    ...
    if (typeof body?.data === "string" && body.data.trim()) {
      const packed = await skills.importArchive(dshHome, Buffer.from(body.data, "base64"), body.name || "skill.zip", options);
      return { ok: true, ...packed };
    }
    if (Array.isArray(body?.files) && body.files.length) {
      const packed = await skills.importFileTree(dshHome, body.files, body.name || "", options);
      return { ok: true, ...packed };
    }
...
function installSkillsHttp(webServer) {
  ...
  for (const op of ["status", "read", "save", "create", "delete", "import", "workspace"]) {
