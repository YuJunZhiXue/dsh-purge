...
async function handlePurgeCommand(rawInput, config) {
  ...
  switch (sub) {
    ...
    case "a": {
      ...
      lines.push("Desktop: 仍写入 host-commands 启动器；备份改放到 $DSH_HOME/dsh-purge/shim-backups。");
      ...
      host.webServer.register({
        ...
        handler: async (request, response) => {
          ...
          try {
            ...
            const applied = report.filter((r) => r.status === "applied").length;
            const already = report.filter((r) => r.status === "already").length;
            // missing_file = optional target not installed (liangshen / web-fetch / …) — not a hard fail.
            const softOk = new Set(["applied", "already", "missing_file"]);
            const failed = report.filter((r) => !softOk.has(r.status));
            // Restart only when required patches finished AND (on win32) bin.js hide-console pinned.
            const complete = failed.length === 0 && flash.ok === true;
            sendJson(response, 200, {
              ok: true,
              complete,
              applied,
