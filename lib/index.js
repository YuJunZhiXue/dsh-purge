...
export function apply(ctx, config) {
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      try {
        ...
        const content = fs.readFileSync(injectFile, "utf8").trim();
        return content;
      } catch {
        return "";
      }
  ...
  if (ctx.tools) {
    ctx.tools.register({
      name: "purge_status",
      description: "查看 dsh 指令权威性清洗状态（8 patch 清洗进度、shim 注入、override 文件）。",
      parameters: {
        type: "object",
    ...
    ctx.tools.register({
      name: "purge_apply",
      description: "应用 dsh 指令权威性清洗：8 patch（提示词层+代码层）+ shim 注入 + override 文件。需重启 dsh 完全生效。",
      parameters: {
        type: "object",
        additionalProperties: false,
        properties: {},
