...
export const TARBALL_URL = `https://github.com/${REPO}/archive/refs/heads/${BRANCH}.tar.gz`;
...
export function pluginAddSpec(spec) {
  const s = String(spec || "");
  if (/\.(zip|tgz|tar\.gz)(\b|$)/i.test(s) || /^https?:\/\//i.test(s)) return TARBALL_URL;
  if (/^github:/i.test(s) || /github\.com\/.+\.git/i.test(s)) return GITHUB_SPEC;
  return "";
}

function dshReinstall(profile, spec) {
  const addSpec = pluginAddSpec(spec);
  if (addSpec) {
  ...
  try {
    ...
    runDshPlugin(profile, ["add", "--force", TARBALL_URL]);
  }
}
