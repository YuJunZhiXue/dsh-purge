...
async function listRemoteVersions() {
  ...
  return {
    versions: out.map((item) => ({
      ...item,
      shaShort: item.sha ? String(item.sha).slice(0, 7) : "",
      current: Boolean(
...
export async function checkUpdate() {
  ...
  const remoteSha = state.channel === "beta" ? listed.betaSha : listed.stableSha;
  const remoteVer = remoteSha ? await remotePackageVersion(remoteSha, channelRef) : "";
  if (!remoteSha && !remoteVer) {
    if (state.channel === "beta") {
      return rememberUpdate({
        ...
        remoteVersion: "",
        localSha: ((gitOk() ? gitHead(PLUGIN_ROOT) : "") || readStamp()).slice(0, 7),
        remoteSha: "",
        via: detectVia(),
    ...
    throw new Error("无法读取 GitHub master");
  }
  ...
  let hasUpdate = false;
  if (state.pin) {
    hasUpdate = Boolean(remoteSha && localRev && !sameRev(localRev, remoteSha));
...
function gitResetTo(dir, ref, { force = false } = {}) {
  ...
  try {
    if (target === STABLE_REF || target === BETA_REF) {
      runGit(dir, ["pull", "--ff-only", "origin", target], 60000);
      return;
    }
  } catch {
    // 再走 fetch + reset。
  ...
  const remote = (target === STABLE_REF || target === BETA_REF) ? `origin/${target}` : target;
  runGit(dir, ["reset", "--hard", remote], 60000);
}
...
export async function switchChannel(channel, { apply = true } = {}) {
  ...
  if (next === "beta") {
    ...
    if (!hasBeta) {
      const stable = writeChannelState({ channel: "stable", pin: "" });
      return rememberUpdate({
        ok: false,
        error: "还没有测试版",
        hasBeta: false,
        ...channelPublic(stable),
        versions: [],
      });
    }
    if (!betaSha && apply) {
  ...
  if (!apply) return checkUpdate();
  return applyUpdate(planUpdates(), { ref: currentChannelRef(), channel: next, pin: false, force: true });
}
...
export async function handleUpdateOp(body = {}) {
  ...
  return applyUpdate(planUpdates(), { ref: body.ref || currentChannelRef(), pin: false, force: true });
}
...
export async function autoUpdateIfNeeded() {
  try {
    ...
    const applied = await applyUpdate(runnable, { ref: currentChannelRef(), pin: false, force: false });
